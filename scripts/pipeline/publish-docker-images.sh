...
export TAG=rarebirds/"$PROJECT_NAME":"$TRAVIS_BRANCH"
...
then
  ...
  export TAG_BUILD_PRIVATE="$PRIVATE_REGISTRY_BASE/rarebirds/$PROJECT_NAME:t_$TRAVIS_BUILD_ID"
  docker tag "$TAG" "$TAG_BUILD_PRIVATE"
  echo "Pushing [private]/rarebirds/$PROJECT_NAME:$TRAVIS_BUILD_ID"
...
then
  echo "Enter TAG Path"
  export TAG_LATEST=rarebirds/"$PROJECT_NAME":latest
  echo "$DOCKER_PASSWORD" | docker login -u "$DOCKER_USERNAME" --password-stdin
